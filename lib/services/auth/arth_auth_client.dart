...
class AuthClient {
  ...
  static FromStorageResult fromStorage(String str) {
    ...
    final DelegationChain? chain = delegationString != null ? DelegationChain.fromJSON(delegationString) : null;
  ...
  Future<void> login([AuthClientLoginOptions? options, String? provider]) async {
    key ??= await Ed25519KeyIdentity.generate(null);
    // Create the URL of the IDP. (e.g. https://XXXX/#authorize)
    final payload = _createAuthPayload(options, provider);
    final result = await authFunction(payload);
    final parsedResult = Uri.parse(result);
    if ((parsedResult.queryParameters['success'] is String && parsedResult.queryParameters['success'] == 'false') ||
  ...
  AuthPayload _createAuthPayload(AuthClientLoginOptions? options, String? provider) {
    ...
    final identityProviderUrl = Uri(
      ...
      queryParameters: {
        ...
        'sessionPublicKey': key != null ? (key as SignIdentity).getPublicKey().toDer().toHex() : null,
  ...
  String toStorage() {
    return jsonEncode(
      {
        keyLocalStorageKey: key != null ? jsonEncode((key as Ed25519KeyIdentity).toJson()) : null,
        keyLocalStorageDelegation: chain != null ? jsonEncode(chain!.toJson()) : null,
